...
Public Sub ExportSCLX()
    ...
    root.Add "supplementalItems", ExportSupplementalItems()
        ...
        If IsLedgerRowUsed(wsL, r) Then
            For i = LBound(ledgerSplitCols) To UBound(ledgerSplitCols)
                ...
                If Len(incomeCat) > 0 Then
                    AddSimpleAccount accounts, seen, incomeCat
                End If
                If Len(expenseCat) > 0 Then
                    AddSimpleAccount accounts, seen, expenseCat
                End If
            Next i
        End If
    Next r
...
Private Function ExportLedgerRowAsTransaction(ws As Worksheet, ByVal r As Long) As Object
    ...
    If Len(SafeText(ws.Cells(r, COL_LEDGER_REF).Value)) > 0 Then tx("referenceId") = ReferenceIdFromText(ws.Cells(r, COL_LEDGER_REF).Value)
...
'
' Notes
'   Returns Null when the source reference text is blank. The raw workbook text remains in transaction.reference.
'------------------------------------------------------------------------------
