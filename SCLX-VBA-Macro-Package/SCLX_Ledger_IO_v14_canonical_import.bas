...
Private Const SH_ASSET_DETAILS As String = "AssetDetails"
Private Const SH_LIABILITY_DETAILS As String = "LiabilityDetails"

Private Const ROW_LEDGER_FIRST As Long = 9
Private Const ROW_OUT_FIRST As Long = 14
Private Const ROW_OUT_LAST As Long = 33
Private Const ROW_ASSET_FIRST As Long = 11
Private Const ROW_SUPPLY_FIRST As Long = 10
Private Const ROW_AD_REC_FIRST As Long = 14
Private Const ROW_AD_REC_LAST As Long = 23
Private Const ROW_AD_PRE_FIRST As Long = 31
Private Const ROW_AD_PRE_LAST As Long = 51
Private Const ROW_AD_OTH_FIRST As Long = 59
Private Const ROW_AD_OTH_LAST As Long = 66
Private Const ROW_LD_DEF_FIRST As Long = 12
Private Const ROW_LD_DEF_LAST As Long = 30
Private Const ROW_LD_PAY_FIRST As Long = 38
Private Const ROW_LD_PAY_LAST As Long = 47
Private Const ROW_LD_OTH_FIRST As Long = 54
Private Const ROW_LD_OTH_LAST As Long = 62
...
Private mExportCashAccountName As String
Private mSkippedNonpostingTransactions As Long
Private mSkippedOutsideReportingPeriod As Long
Private mExportedSupplementalDetails As Long
Private mSkippedUnlinkedSupplementalDetails As Long
...
' AssetDetails / LiabilityDetails supplemental schedule columns
Private Const COL_DET_COUNTERPARTY As String = "B"
Private Const COL_DET_YEAR As String = "C"
Private Const COL_DET_REASON As String = "D"
Private Const COL_DET_SUBTYPE As String = "E"
Private Const COL_DET_BUDGET As String = "F"
Private Const COL_DET_SOURCE As String = "G"
Private Const COL_DET_AMOUNT As String = "H"
Private Const COL_DET_LEDGERROW As String = "I"
...
Public Sub ExportSCLX()
    ...
    InitSplitCols
    mSkippedNonpostingTransactions = 0
    mSkippedOutsideReportingPeriod = 0
    mExportedSupplementalDetails = 0
    mSkippedUnlinkedSupplementalDetails = 0
    ...
    ' Ask once for the balancing Cash account used by every unbalanced
    ' Ledger transaction in this export.
    Dim cashAccountResponse As Variant
    mExportCashAccountName = ""
    cashAccountResponse = Application.InputBox( _
        Prompt:="Enter the account name or account number to use as the Cash balancing account for all unbalanced Ledger transactions in this export.", _
        Title:="SCLX Cash Counterparty Account", _
        Default:="Cash", _
        Type:=2)

    If VarType(cashAccountResponse) = vbBoolean Then
        If cashAccountResponse = False Then Exit Sub
    End If

    mExportCashAccountName = Trim$(CStr(cashAccountResponse))
    If Len(mExportCashAccountName) = 0 Then
        MsgBox "A Cash balancing account is required for SCLX export.", vbExclamation
        Exit Sub
    End If
    ...
    ' Workbook budget labels are annotations, not canonical budget plans with
    ' amounts. Empty donor shells are therefore omitted from the clean export.
    root.Add "budgets", NewJsonArray()
    ...
    root.Add "outstandingItems", NewJsonArray()
    root.Add "otherAssetItems", NewJsonArray()
    root.Add "supplementalItems", NewJsonArray()
    root.Add "assets", NewJsonArray()
    root.Add "supplies", NewJsonArray()
    root.Add "bankStatementImports", NewJsonArray()
    root.Add "extensions", ExportRootExtensions()

    WriteTextFile CStr(path), JsonConverter.ConvertToJson(root, Whitespace:=2)
    MsgBox "SCLX export written to:" & vbCrLf & CStr(path) & vbCrLf & vbCrLf & _
           "Clean-import reconciliation:" & vbCrLf & _
           "- Cash balancing entries are lines within their owning transactions." & vbCrLf & _
           "- Skipped non-posting workbook annotation rows: " & mSkippedNonpostingTransactions & vbCrLf & _
           "- Skipped transactions outside the reporting period: " & mSkippedOutsideReportingPeriod & vbCrLf & _
           "- Exported transaction-linked supplemental details: " & mExportedSupplementalDetails & vbCrLf & _
           "- Skipped unlinked supplemental schedule rows: " & mSkippedUnlinkedSupplementalDetails & vbCrLf & _
           "- Outstanding, fixed-asset, and supplies schedules were excluded because this workbook does not contain the canonical fields required for a lossless import.", _
           vbInformation
    mExportCashAccountName = ""
    Exit Sub
...
Public Sub ImportSCLX()
    ...
    If root.Exists("supplementalItems") Then ImportSupplementalItems root("supplementalItems")
...
Private Function ExportOrganization() As Object
    ...
    d("code") = UCase$(Replace(NormalizeId("", nm), "-", "_"))
    d("name") = nm
...
'------------------------------------------------------------------------------
Private Function ExportRootExtensions() As Object
    Dim ext As Object
    Dim app As Object
    Dim ws As Worksheet
    ...
    organizationName = SafeText(ws.Range(CELL_SUM_ORG_NAME).Value)
    If Len(organizationName) = 0 Then organizationName = "Workbook"
    app("activeChartName") = organizationName & " Chart of Accounts"
    app("activeChartVersion") = "workbook-" & CStr(SummaryYear(ws))
    app.Add "supplementalDetails", ExportCanonicalSupplementalDetails()

    ext.Add "scaJakartaH2", app
    Set ExportRootExtensions = ext
End Function
...
Private Function ExportChartOfAccounts() As Collection
    ...
    ' The synthesized balancing line references this account, so ensure it is
    ' present in chartOfAccounts even though it does not appear in a workbook
    ' income/expense split category.
    AddCashAccount accounts, seen, mExportCashAccountName

    For r = ROW_LEDGER_FIRST To FindLastInterestingLedgerRow(wsL)
        If IsExportableLedgerTransaction(wsL, r, False) Then
            For i = LBound(ledgerSplitCols) To UBound(ledgerSplitCols)
                grp = ledgerSplitCols(i)

                incomeCat = SafeText(wsL.Cells(r, grp(2)).Value)
                expenseCat = SafeText(wsL.Cells(r, grp(3)).Value)

                If Len(incomeCat) > 0 Then
                    AddTypedAccount accounts, seen, incomeCat, "INCOME"
                End If
                If Len(expenseCat) > 0 Then
                    AddTypedAccount accounts, seen, expenseCat, "EXPENSE"
                End If
            Next i
        End If
    Next r
...
Private Function ExportFunds() As Collection
    ...
    For r = ROW_LEDGER_FIRST To FindLastInterestingLedgerRow(ws)
        If IsExportableLedgerTransaction(ws, r, False) Then
            ...
            If Len(fundName) > 0 Then
                If Not seen.Exists(UCase$(fundName)) Then
                    ...
                    d("code") = UCase$(Replace(NormalizeId("", fundName), "-", "_"))
                    d("name") = fundName
                    d("type") = "UNRESTRICTED"
                    d("active") = True
                    funds.Add d
...
Private Function ExportPeople() As Collection
    ...
    Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_AD_REC_FIRST, ROW_AD_REC_LAST
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_AD_PRE_FIRST, ROW_AD_PRE_LAST
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_AD_OTH_FIRST, ROW_AD_OTH_LAST

    Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_LD_DEF_FIRST, ROW_LD_DEF_LAST
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_LD_PAY_FIRST, ROW_LD_PAY_LAST
    AddSimplePeopleFromSupplementalSection coll, seen, ws, ROW_LD_OTH_FIRST, ROW_LD_OTH_LAST
...
Private Function ExportTransactions() As Collection
    ...
    For r = ROW_LEDGER_FIRST To lastRow
        If IsExportableLedgerTransaction(ws, r, True) Then
...
Private Function ExportLedgerRowAsTransaction(ws As Worksheet, ByVal r As Long) As Object
    ...
    tx("checkNumber") = SafeText(ws.Cells(r, COL_LEDGER_REF).Value)
    If Len(SafeText(ws.Cells(r, COL_LEDGER_REF).Value)) > 0 Then tx("checkNumberId") = ReferenceIdFromText(ws.Cells(r, COL_LEDGER_REF).Value)
    tx("personId") = PersonIdFromName(ws.Cells(r, COL_LEDGER_NAME).Value)
    tx("personDisplayName") = SafeOrNull(ws.Cells(r, COL_LEDGER_NAME).Value)
    tx("status") = "ENTERED"
    tx("source") = "MANUAL"
    ...
    Set lines = ExportLedgerSplitLines(ws, r)
    ' The assumed Cash/Asset counter-line is part of this transaction's line
    ' collection. It is never appended to the top-level transactions array.
    AddSyntheticCashBalancingLine lines, ws, r
    ValidateTransactionLines lines, r
    tx.Add "lines", lines
...
'------------------------------------------------------------------------------
' Purpose
'   Adds one synthetic Cash transaction line when the workbook split lines for
'   a Ledger row are not balanced.
'
' Parameters
'   lines - Collection containing the original exported Ledger split lines.
'   ws - Source Ledger worksheet.
'   r - Worksheet row number.
'
' Notes
'   The account name is requested once at the beginning of ExportSCLX and reused
'   for every transaction. If debits exceed credits, Cash is credited by the
'   difference. If credits exceed debits, Cash is debited by the difference.
'   Rows with no nonzero exported lines do not receive a synthetic line.
'------------------------------------------------------------------------------
Private Sub AddSyntheticCashBalancingLine(lines As Collection, ws As Worksheet, ByVal r As Long)
    If lines Is Nothing Then Exit Sub
    If lines.Count = 0 Then Exit Sub
    If Len(Trim$(mExportCashAccountName)) = 0 Then Exit Sub
    ...
    For Each line In lines
        debitTotal = debitTotal + CCur(ParseJsonNumber(ValueOrFallback(line, "debit", "0")))
        creditTotal = creditTotal + CCur(ParseJsonNumber(ValueOrFallback(line, "credit", "0")))
    Next line

    If debitTotal = creditTotal Then Exit Sub

    lines.Add ExportSyntheticCashLine(ws, r, debitTotal, creditTotal)
End Sub


'------------------------------------------------------------------------------
' Purpose
'   Enforces the transaction-level invariant after the assumed Cash/Asset line
'   has been attached to the owning transaction.
'------------------------------------------------------------------------------
Private Sub ValidateTransactionLines(lines As Collection, ByVal ledgerRow As Long)
    Dim line As Variant
    Dim debitTotal As Currency
    Dim creditTotal As Currency
    Dim postingLines As Long

    For Each line In lines
        debitTotal = debitTotal + CCur(ParseJsonNumber(ValueOrFallback(line, "debit", "0")))
        creditTotal = creditTotal + CCur(ParseJsonNumber(ValueOrFallback(line, "credit", "0")))
        If CCur(ParseJsonNumber(ValueOrFallback(line, "debit", "0"))) <> 0 Or _
           CCur(ParseJsonNumber(ValueOrFallback(line, "credit", "0"))) <> 0 Then
            postingLines = postingLines + 1
        End If
    Next line

    If postingLines < 2 Or debitTotal <> creditTotal Then
        Err.Raise vbObjectError + 1201, , _
            "Ledger row " & ledgerRow & " did not produce one balanced transaction with at least two posting lines."
    End If
End Sub
...
'------------------------------------------------------------------------------
' Purpose
'   Builds the synthetic Cash balancing line for one exported Ledger transaction.
'
' Notes
'   The line is tagged in extensions.workbook.syntheticBalancingLine so the
'   Excel importer can ignore it and restore only the original workbook splits.
'------------------------------------------------------------------------------
Private Function ExportSyntheticCashLine(ws As Worksheet, ByVal r As Long, ByVal debitTotal As Currency, ByVal creditTotal As Currency) As Object
    Dim d As Object
    Dim ext As Object
    Dim wbk As Object
    ...
    Set d = CreateObject("Scripting.Dictionary")
    d("lineId") = "ledger-row-" & CStr(r) & "-cash-balancing"
    d("accountId") = mExportCashAccountName

    If debitTotal > creditTotal Then
        amount = debitTotal - creditTotal
        d("debit") = FormatAmount(0)
        d("credit") = FormatAmount(CDbl(amount))
    Else
        amount = creditTotal - debitTotal
        d("debit") = FormatAmount(CDbl(amount))
        d("credit") = FormatAmount(0)
    End If
    ...
    d("eventId") = Null
    d("documentId") = Null
    d("memo") = "Synthetic Cash balancing line for Ledger row " & CStr(r)
    d("usedFor") = Null
    d("itemNumber") = Null
    d("quantity") = Null
    d.Add "workbookLink", WorkbookLinkObject(SH_LEDGER, r)
    ...
    Set ext = CreateObject("Scripting.Dictionary")
    Set wbk = CreateObject("Scripting.Dictionary")
    wbk("syntheticBalancingLine") = True
    wbk("syntheticRole") = "CASH"
    wbk("cashAccount") = mExportCashAccountName
    wbk("amount") = FormatAmount(CDbl(amount))
    ext.Add "workbook", wbk
    d.Add "extensions", ext

    Set ExportSyntheticCashLine = d
End Function


'------------------------------------------------------------------------------
' Purpose
'   Returns True when an SCLX line is an exporter-generated Cash balancing line.
'
' Notes
'   Such lines belong in the canonical transaction/JAVA journal but do not map
'   to an original Excel split block, so Excel import skips them.
'------------------------------------------------------------------------------
Private Function IsSyntheticBalancingLine(line As Variant) As Boolean
    On Error GoTo Nope
    ...
    If HasWorkbookExtension(line) Then
        ...
        If ExistsInDict(wbk, "syntheticBalancingLine") Then
            IsSyntheticBalancingLine = CBool(ValueOrFallback(wbk, "syntheticBalancingLine", False))
            Exit Function
        End If
    End If

Nope:
    IsSyntheticBalancingLine = False
End Function
...
'------------------------------------------------------------------------------
' Purpose
'   Exports the bounded supplemental schedule sections from AssetDetails and LiabilityDetails.
'
' Returns
'   Collection of SCLX supplementalItems records preserving workbook section provenance.
'------------------------------------------------------------------------------
Private Function ExportSupplementalItems() As Collection
    Dim coll As New Collection
    Dim ws As Worksheet

    Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
    AppendSupplementalSectionItems coll, ws, "Receivables", "RECEIVABLE", ROW_AD_REC_FIRST, ROW_AD_REC_LAST
    AppendSupplementalSectionItems coll, ws, "Prepaid Expenses", "PREPAID_EXPENSE", ROW_AD_PRE_FIRST, ROW_AD_PRE_LAST
    AppendSupplementalSectionItems coll, ws, "Other Assets", "OTHER_ASSET", ROW_AD_OTH_FIRST, ROW_AD_OTH_LAST

    Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
    AppendSupplementalSectionItems coll, ws, "Deferred Revenue", "DEFERRED_REVENUE", ROW_LD_DEF_FIRST, ROW_LD_DEF_LAST
    AppendSupplementalSectionItems coll, ws, "Payables", "PAYABLE", ROW_LD_PAY_FIRST, ROW_LD_PAY_LAST
    AppendSupplementalSectionItems coll, ws, "Other Liabilities", "OTHER_LIABILITY", ROW_LD_OTH_FIRST, ROW_LD_OTH_LAST

    Set ExportSupplementalItems = coll
End Function


'------------------------------------------------------------------------------
' Purpose
'   Exports only supplemental schedule rows that identify an exported canonical
'   transaction. Unlinked opening schedules remain in the workbook and are
'   disclosed in the export summary instead of being guessed into transactions.
'------------------------------------------------------------------------------
Private Function ExportCanonicalSupplementalDetails() As Collection
    Dim coll As New Collection
    ...
    Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
    AppendCanonicalSupplementalDetails coll, orders, ws, "RECEIVABLE", ROW_AD_REC_FIRST, ROW_AD_REC_LAST
    AppendCanonicalSupplementalDetails coll, orders, ws, "PREPAID_EXPENSE", ROW_AD_PRE_FIRST, ROW_AD_PRE_LAST
    AppendCanonicalSupplementalDetails coll, orders, ws, "OTHER_ASSET", ROW_AD_OTH_FIRST, ROW_AD_OTH_LAST

    Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
    AppendCanonicalSupplementalDetails coll, orders, ws, "DEFERRED_REVENUE", ROW_LD_DEF_FIRST, ROW_LD_DEF_LAST
    AppendCanonicalSupplementalDetails coll, orders, ws, "PAYABLE", ROW_LD_PAY_FIRST, ROW_LD_PAY_LAST
    AppendCanonicalSupplementalDetails coll, orders, ws, "OTHER_LIABILITY", ROW_LD_OTH_FIRST, ROW_LD_OTH_LAST

    Set ExportCanonicalSupplementalDetails = coll
End Function


Private Sub AppendCanonicalSupplementalDetails(ByRef coll As Collection, ByVal orders As Object, ByVal ws As Worksheet, ByVal kind As String, ByVal firstRow As Long, ByVal lastRow As Long)
    Dim r As Long
    Dim ledgerRow As Long
    Dim visibleLedgerRow As String
    Dim transactionId As String
    Dim lineOrder As Long

    For r = firstRow To lastRow
        If IsSupplementalDetailRowUsed(ws, r) And Abs(ValZero(ws.Cells(r, COL_DET_AMOUNT).Value)) > 0 Then
            visibleLedgerRow = SafeText(ws.Cells(r, COL_DET_LEDGERROW).Value)
            ledgerRow = ResolveLedgerWorksheetRow(visibleLedgerRow)
            If ledgerRow = 0 Then
                mSkippedUnlinkedSupplementalDetails = mSkippedUnlinkedSupplementalDetails + 1
            Else
                transactionId = "ledger-row-" & CStr(ledgerRow)
                If orders.Exists(transactionId) Then
                    lineOrder = CLng(orders(transactionId))
                Else
                    lineOrder = 0
                End If
                coll.Add ExportCanonicalSupplementalDetail(ws, r, kind, transactionId, lineOrder, visibleLedgerRow)
                orders(transactionId) = lineOrder + 1
                mExportedSupplementalDetails = mExportedSupplementalDetails + 1
            End If
        End If
    Next r
End Sub
...
Private Function ExportCanonicalSupplementalDetail(ByVal ws As Worksheet, ByVal r As Long, ByVal kind As String, ByVal transactionId As String, ByVal lineOrder As Long, ByVal visibleLedgerRow As String) As Object
    Dim d As Object
    Dim description As String
    Dim notes As String
    Set d = CreateObject("Scripting.Dictionary")

    description = SafeText(ws.Cells(r, COL_DET_REASON).Value)
    If Len(description) = 0 Then description = Replace(kind, "_", " ") & " schedule detail"
    notes = "Workbook " & ws.Name & " row " & CStr(r)
    If Len(SafeText(ws.Cells(r, COL_DET_YEAR).Value)) > 0 Then notes = notes & "; year " & SafeText(ws.Cells(r, COL_DET_YEAR).Value)
    If Len(SafeText(ws.Cells(r, COL_DET_SOURCE).Value)) > 0 Then notes = notes & "; source " & SafeText(ws.Cells(r, COL_DET_SOURCE).Value)
    If Len(SafeText(ws.Cells(r, COL_DET_SUBTYPE).Value)) > 0 Then notes = notes & "; subtype " & SafeText(ws.Cells(r, COL_DET_SUBTYPE).Value)

    d("supplementalDetailId") = "supplemental-detail:" & transactionId & ":" & CStr(lineOrder + 1)
    d("transactionId") = transactionId
    d("lineOrder") = lineOrder
    d("kind") = kind
    d("entryRef") = visibleLedgerRow
    d("counterparty") = SafeOrNull(ws.Cells(r, COL_DET_COUNTERPARTY).Value)
    d("reference") = SafeOrNull(ws.Cells(r, COL_DET_BUDGET).Value)
    d("notes") = notes
    d("description") = description
    d("amount") = FormatAmount(Abs(CDbl(ValZero(ws.Cells(r, COL_DET_AMOUNT).Value))))
    d("dueDate") = Null
    d("startDate") = Null
    d("endDate") = Null
    Set ExportCanonicalSupplementalDetail = d
End Function


Private Function ResolveLedgerWorksheetRow(ByVal visibleLedgerRow As String) As Long
    If Len(visibleLedgerRow) = 0 Or Not IsNumeric(visibleLedgerRow) Then Exit Function

    Dim ws As Worksheet
    Dim r As Long
    Set ws = ThisWorkbook.Worksheets(SH_LEDGER)
    For r = ROW_LEDGER_FIRST To FindLastInterestingLedgerRow(ws)
        If SafeText(ws.Cells(r, COL_LEDGER_ROWNUM).Value) = CStr(CLng(visibleLedgerRow)) Then
            If IsExportableLedgerTransaction(ws, r, False) Then ResolveLedgerWorksheetRow = r
            Exit Function
        End If
    Next r
End Function
...
'------------------------------------------------------------------------------
' Purpose
'   Appends one bounded detail section to the supplementalItems export collection.
'------------------------------------------------------------------------------
Private Sub AppendSupplementalSectionItems(ByRef coll As Collection, ByVal ws As Worksheet, ByVal sectionName As String, ByVal kind As String, ByVal firstRow As Long, ByVal lastRow As Long)
    Dim r As Long

    For r = firstRow To lastRow
        If IsSupplementalDetailRowUsed(ws, r) Then
            coll.Add ExportOneSupplementalItem(ws, sectionName, kind, r)
        End If
    Next r
End Sub
...
'------------------------------------------------------------------------------
' Purpose
'   Exports one supplemental schedule row from AssetDetails or LiabilityDetails.
'------------------------------------------------------------------------------
Private Function ExportOneSupplementalItem(ByVal ws As Worksheet, ByVal sectionName As String, ByVal kind As String, ByVal r As Long) As Object
    Dim d As Object
    Dim ext As Object
    Dim wbk As Object
    ...
    Set d = CreateObject("Scripting.Dictionary")
    counterpartyName = SafeText(ws.Cells(r, COL_DET_COUNTERPARTY).Value)

    d("supplementalItemId") = NormalizeId("supp-", ws.Name & "-" & kind & "-" & CStr(r))
    d("kind") = kind
    d("counterpartyName") = SafeOrNull(counterpartyName)
    d("personId") = PersonIdFromName(counterpartyName)
    d("year") = NullOrLong(ws.Cells(r, COL_DET_YEAR).Value)
    d("reason") = SafeOrNull(ws.Cells(r, COL_DET_REASON).Value)
    d("subtypeCode") = SafeOrNull(ws.Cells(r, COL_DET_SUBTYPE).Value)
    d("eventBudgetLabel") = SafeOrNull(ws.Cells(r, COL_DET_BUDGET).Value)
    d("budgetId") = Null
    d("sourceLabel") = SafeOrNull(ws.Cells(r, COL_DET_SOURCE).Value)
    d("amountAsOf") = AmountOrNull(ws.Cells(r, COL_DET_AMOUNT).Value)
    d("ledgerRowIndex") = NullOrLong(ws.Cells(r, COL_DET_LEDGERROW).Value)
    d.Add "workbookLink", WorkbookLinkObject(ws.Name, r)

    Set ext = CreateObject("Scripting.Dictionary")
    Set wbk = CreateObject("Scripting.Dictionary")
    wbk("sheet") = ws.Name
    wbk("row") = r
    wbk("section") = sectionName
    ext.Add "workbook", wbk
    d.Add "extensions", ext

    Set ExportOneSupplementalItem = d
End Function
...
Private Sub WriteTransactionToLedgerRow(ws As Worksheet, ByVal r As Long, tx As Variant)
    ...
    If Len(SafeText(ValueOrFallback(tx, "checkNumber", ""))) > 0 Then
        ws.Cells(r, COL_LEDGER_REF).Value = ValueOrFallback(tx, "checkNumber", "")
    Else
    ...
    Else
        ...
        If ExistsInDict(tx, "lines") Then
            ...
            If Len(SafeText(ValueOrFallback(tx, "personDisplayName", ""))) > 0 Then
                ws.Cells(r, COL_LEDGER_NAME).Value = ValueOrFallback(tx, "personDisplayName", "")
            ElseIf Len(SafeText(ValueOrFallback(tx, "personId", ""))) > 0 Then
                ws.Cells(r, COL_LEDGER_NAME).Value = LookupPersonName(SafeText(ValueOrFallback(tx, "personId", "")))
                If Len(SafeText(ws.Cells(r, COL_LEDGER_NAME).Value)) = 0 Then ws.Cells(r, COL_LEDGER_NAME).Value = ValueOrFallback(tx, "personId", "")
            Else
                ws.Cells(r, COL_LEDGER_NAME).Value = FirstPersonNameFromLines(lines)
            End If
        Else
            If Len(SafeText(ValueOrFallback(tx, "personDisplayName", ""))) > 0 Then ws.Cells(r, COL_LEDGER_NAME).Value = ValueOrFallback(tx, "personDisplayName", "")
        End If
    End If
    ...
    If ExistsInDict(tx, "lines") Then
        ...
        i = 0
        For Each line In lines
            If Not IsSyntheticBalancingLine(line) Then
...
'------------------------------------------------------------------------------
' Purpose
'   Imports SCLX supplementalItems into the bounded AssetDetails and LiabilityDetails sections.
'------------------------------------------------------------------------------
Private Sub ImportSupplementalItems(items As Variant)
    Dim item As Variant

    For Each item In items
        WriteSupplementalItemToSheet item
    Next item
End Sub

'------------------------------------------------------------------------------
' Purpose
'   Routes one supplemental item to the correct workbook section and writes it to the next blank row.
'------------------------------------------------------------------------------
Private Sub WriteSupplementalItemToSheet(item As Variant)
    Dim ws As Worksheet
    ...
    kind = UCase$(SafeText(ValueOrFallback(item, "kind", "")))

    Select Case kind
        Case "RECEIVABLE"
            Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
            firstRow = ROW_AD_REC_FIRST
            lastRow = ROW_AD_REC_LAST
        Case "PREPAID_EXPENSE"
            Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
            firstRow = ROW_AD_PRE_FIRST
            lastRow = ROW_AD_PRE_LAST
        Case "OTHER_ASSET"
            Set ws = ThisWorkbook.Worksheets(SH_ASSET_DETAILS)
            firstRow = ROW_AD_OTH_FIRST
            lastRow = ROW_AD_OTH_LAST
        Case "DEFERRED_REVENUE"
            Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
            firstRow = ROW_LD_DEF_FIRST
            lastRow = ROW_LD_DEF_LAST
        Case "PAYABLE"
            Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
            firstRow = ROW_LD_PAY_FIRST
            lastRow = ROW_LD_PAY_LAST
        Case "OTHER_LIABILITY"
            Set ws = ThisWorkbook.Worksheets(SH_LIABILITY_DETAILS)
            firstRow = ROW_LD_OTH_FIRST
            lastRow = ROW_LD_OTH_LAST
        Case Else
            Exit Sub
    End Select

    r = NextBlankSupplementalRow(ws, firstRow, lastRow)

    ws.Cells(r, COL_DET_COUNTERPARTY).Value = ValueOrFallback(item, "counterpartyName", "")
    If Len(SafeText(ws.Cells(r, COL_DET_COUNTERPARTY).Value)) = 0 Then
        ws.Cells(r, COL_DET_COUNTERPARTY).Value = LookupPersonName(SafeText(ValueOrFallback(item, "personId", "")))
    End If

    If Not IsNull(ValueOrFallback(item, "year", Null)) And Len(SafeText(ValueOrFallback(item, "year", Null))) > 0 Then
        ws.Cells(r, COL_DET_YEAR).Value = ValueOrFallback(item, "year", Null)
    End If

    ws.Cells(r, COL_DET_REASON).Value = ValueOrFallback(item, "reason", "")
    ws.Cells(r, COL_DET_SUBTYPE).Value = ValueOrFallback(item, "subtypeCode", "")
    ws.Cells(r, COL_DET_BUDGET).Value = ValueOrFallback(item, "eventBudgetLabel", "")
    If Len(SafeText(ws.Cells(r, COL_DET_BUDGET).Value)) = 0 Then
        ws.Cells(r, COL_DET_BUDGET).Value = LookupBudgetName(SafeText(ValueOrFallback(item, "budgetId", "")))
    End If

    ws.Cells(r, COL_DET_SOURCE).Value = ValueOrFallback(item, "sourceLabel", "")

    If Len(SafeText(ValueOrFallback(item, "amountAsOf", ""))) > 0 Then
        ws.Cells(r, COL_DET_AMOUNT).Value = ParseJsonNumber(ValueOrFallback(item, "amountAsOf", "0"))
    End If

    If Not IsNull(ValueOrFallback(item, "ledgerRowIndex", Null)) And Len(SafeText(ValueOrFallback(item, "ledgerRowIndex", Null))) > 0 Then
        ws.Cells(r, COL_DET_LEDGERROW).Value = ValueOrFallback(item, "ledgerRowIndex", Null)
    End If
End Sub
...
Private Function IsLedgerRowUsed(ws As Worksheet, ByVal r As Long) As Boolean
    IsLedgerRowUsed = RowHasAnyValue(ws, r, Array(COL_LEDGER_TXN_DATE, COL_LEDGER_DATE_SHOWS, COL_LEDGER_REF, COL_LEDGER_INCOMING))
End Function


'------------------------------------------------------------------------------
' Purpose
'   Determines whether a workbook row is a canonical posting transaction for
'   the Summary reporting period.
'------------------------------------------------------------------------------
Private Function IsExportableLedgerTransaction(ws As Worksheet, ByVal r As Long, ByVal countSkip As Boolean) As Boolean
    If Not IsLedgerRowUsed(ws, r) Then Exit Function

    If Not IsDate(ws.Cells(r, COL_LEDGER_TXN_DATE).Value) Or Not HasNonzeroPostingSplit(ws, r) Then
        If countSkip Then mSkippedNonpostingTransactions = mSkippedNonpostingTransactions + 1
        Exit Function
    End If
    ...
    Dim endDate As Date
    Set summary = ThisWorkbook.Worksheets(SH_SUMMARY)
    startDate = DateValueOrFallback(summary.Range(CELL_SUM_REPORT_START).Value, _
                                    DateSerial(SummaryYear(summary), 1, 1))
    endDate = DateValueOrFallback(summary.Range(CELL_SUM_REPORT_END).Value, _
                                  DateSerial(SummaryYear(summary), 12, 31))

    If CDate(ws.Cells(r, COL_LEDGER_TXN_DATE).Value) < startDate Or _
       CDate(ws.Cells(r, COL_LEDGER_TXN_DATE).Value) > endDate Then
        If countSkip Then mSkippedOutsideReportingPeriod = mSkippedOutsideReportingPeriod + 1
        Exit Function
    End If

    IsExportableLedgerTransaction = True
End Function


Private Function HasNonzeroPostingSplit(ws As Worksheet, ByVal r As Long) As Boolean
    Dim i As Long
    Dim grp As Variant
    For i = LBound(ledgerSplitCols) To UBound(ledgerSplitCols)
        grp = ledgerSplitCols(i)
        If Abs(ValZero(ws.Cells(r, grp(1)).Value)) > 0 And _
           (Len(SafeText(ws.Cells(r, grp(2)).Value)) > 0 Or _
            Len(SafeText(ws.Cells(r, grp(3)).Value)) > 0) Then
            HasNonzeroPostingSplit = True
            Exit Function
        End If
    Next i
End Function
...
'------------------------------------------------------------------------------
Private Sub AddCashAccount(coll As Collection, seen As Object, ByVal nameOrId As String)
...
Private Sub AddTypedAccount(coll As Collection, seen As Object, ByVal nameOrId As String, ByVal accountType As String)
    If Len(Trim$(nameOrId)) = 0 Then Exit Sub
    ...
    Set d = CreateObject("Scripting.Dictionary")
    resolvedType = CanonicalAccountTypeForUsage(nameOrId, accountType)
    d("accountId") = nameOrId
    d("code") = nameOrId
    d("name") = nameOrId
    d("type") = resolvedType
...
Private Function CanonicalAccountTypeForUsage(ByVal accountName As String, ByVal usageType As String) As String
    Dim normalizedName As String
    normalizedName = UCase$(SafeText(accountName))

    ' These workbook labels represent returnable assets even when the source
    ' template places their debit/credit input in an expense/income column.
    If InStr(normalizedName, "CASH ADVANCE") > 0 Or _
       InStr(normalizedName, "SECURITY DEPOSIT RETURN") > 0 Then
        CanonicalAccountTypeForUsage = "ASSET"
    Else
        CanonicalAccountTypeForUsage = UCase$(usageType)
    End If
End Function
...
'------------------------------------------------------------------------------
' Purpose
'   Normalizes workbook reference/check text into a stable referenceId value.
'
' Parameters
'   refValue - Workbook reference/check/transfer text used to build a stable identifier.
'
' Returns
'   Value described by the procedure name and summary above.
'
' Notes
'   Returns Null when the source reference text is blank. The raw workbook text remains in transaction.checkNumber.
'------------------------------------------------------------------------------
Private Function ReferenceIdFromText(ByVal refValue As Variant) As Variant
    Dim s As String
    s = SafeText(refValue)
    If Len(s) = 0 Then
        ReferenceIdFromText = Null
    Else
        ReferenceIdFromText = NormalizeId("ref-", s)
    End If
End Function
...
Private Function SafeText(v As Variant) As String
    If IsError(v) Or IsNull(v) Or IsEmpty(v) Then
        SafeText = ""
    Else
        SafeText = Trim$(Replace(Replace(CStr(v), ChrW(160), " "), vbTab, " "))
    End If
End Function
...
Private Function NullOrLong(v As Variant) As Variant
    If Len(SafeText(v)) = 0 Then
        NullOrLong = Null
    ElseIf IsNumeric(v) Then
        NullOrLong = CLng(v)
    ElseIf IsNumeric(SafeText(v)) Then
        NullOrLong = CLng(Val(SafeText(v)))
    Else
        NullOrLong = Null
    End If
End Function
...
'------------------------------------------------------------------------------
' Purpose
'   Returns a locale-independent VBA Date from a workbook cell or fallback.
'------------------------------------------------------------------------------
Private Function DateValueOrFallback(v As Variant, ByVal fallbackDate As Date) As Date
    If IsDate(v) Then
        DateValueOrFallback = CDate(v)
    Else
        DateValueOrFallback = fallbackDate
    End If
End Function
...
Private Function FindLastInterestingOutstandingRow(ws As Worksheet) As Long
    FindLastInterestingOutstandingRow = ROW_OUT_LAST
End Function
...
Private Function IsOutstandingRowUsed(ws As Worksheet, ByVal r As Long) As Boolean
    If r < ROW_OUT_FIRST Or r > ROW_OUT_LAST Then
        IsOutstandingRowUsed = False
    ElseIf Abs(ValZero(ws.Cells(r, COL_OUT_AMOUNT).Value)) > 0 Then
        IsOutstandingRowUsed = True
    Else
        IsOutstandingRowUsed = RowHasAnyValue(ws, r, Array(COL_OUT_DATE_SENT, COL_OUT_INCOMING_DATE, COL_OUT_TRANSFER_OR_CHECK, COL_OUT_DATE_SHOWS, COL_OUT_NAME, COL_OUT_DETAILS, COL_OUT_MERCHANT, COL_OUT_ACCOUNT, COL_OUT_DATE_REVERSED, COL_OUT_REASON_APPROVAL))
    End If
End Function
...
Private Function NextOutstandingAppendRow(ws As Worksheet) As Long
    ...
    For r = ROW_OUT_FIRST To ROW_OUT_LAST
    ...
    Err.Raise vbObjectError + 1100, , "Outstanding sheet data block is full (valid rows are 14 through 33)."
End Function
...
'------------------------------------------------------------------------------
' Purpose
'   Determines whether a bounded supplemental detail row contains real user data.
'------------------------------------------------------------------------------
Private Function IsSupplementalDetailRowUsed(ByVal ws As Worksheet, ByVal r As Long) As Boolean
    If RowHasAnyValue(ws, r, Array(COL_DET_COUNTERPARTY, COL_DET_YEAR, COL_DET_REASON, COL_DET_SUBTYPE, COL_DET_BUDGET, COL_DET_SOURCE, COL_DET_LEDGERROW)) Then
        IsSupplementalDetailRowUsed = True
    ElseIf Abs(ValZero(ws.Cells(r, COL_DET_AMOUNT).Value)) > 0 Then
        IsSupplementalDetailRowUsed = True
    Else
        IsSupplementalDetailRowUsed = False
    End If
End Function

'------------------------------------------------------------------------------
' Purpose
'   Finds the next blank row within one accepted supplemental detail section.
'------------------------------------------------------------------------------
Private Function NextBlankSupplementalRow(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long) As Long
    Dim r As Long

    For r = firstRow To lastRow
        If Not IsSupplementalDetailRowUsed(ws, r) Then
            NextBlankSupplementalRow = r
            Exit Function
        End If
    Next r

    Err.Raise vbObjectError + 1110, , ws.Name & " section data block is full (" & firstRow & ":" & lastRow & ")."
End Function

'------------------------------------------------------------------------------
' Purpose
'   Adds person records from one bounded supplemental detail section.
'------------------------------------------------------------------------------
Private Sub AddSimplePeopleFromSupplementalSection(ByRef coll As Collection, ByVal seen As Object, ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long)
    Dim r As Long

    For r = firstRow To lastRow
        If IsSupplementalDetailRowUsed(ws, r) And _
           ResolveLedgerWorksheetRow(SafeText(ws.Cells(r, COL_DET_LEDGERROW).Value)) > 0 Then
            AddSimplePerson coll, seen, ws.Cells(r, COL_DET_COUNTERPARTY).Value, Null, Null, "OTHER"
        End If
    Next r
End Sub
