...
## 12. Supplemental schedule items
...
## 21. Practical note for spreadsheet integrations
...
## Transaction Person and Reference Dual Storage

For workbook-oriented ledger bridges, a transaction may carry both a normalized identifier and the original workbook text for person/name and reference/check fields.

Recommended transaction fields are:

- `personId`: normalized identifier derived from the workbook person/payee field
- `personDisplayName`: original workbook text for the person/payee field
- `reference`: original workbook reference/check/transfer text
- `referenceId`: normalized identifier derived from `reference`

This dual storage is intended to make translation easier across systems that may prefer either normalized identifiers or original workbook text. Exporters should preserve the original workbook text whenever available and may additionally emit normalized identifiers for indexing and cross-reference purposes.

Importers should prefer the original text field when writing back to workbook cells. If the original text is absent, importers may fall back to the normalized identifier or to lookup tables such as the top-level `people` collection.
